...
offsetHub = np.zeros((res,res+4,3))
...
XhO = np.zeros([res, res+4])
YhO = np.zeros([res, res+4])
ZhO = np.zeros([res, res+4])
XcO = np.zeros([res, res+4])
YcO = np.zeros([res, res+4])
ZcO = np.zeros([res, res+4])
for d in range(res):
...
rotHub = np.zeros((res, len(hub), 3))
rotCas = np.zeros((res, len(cas), 3))
...
for qq in range(len(Xvalues)):
    filename = '../thinSliceStatorOnly/constant/geometry/{}.stl'.format(filenames[qq])
...
f = open('../thinSliceStatorOnly/system/pointData', 'w')
