...
res = 360
length = 20
Nb = 180 #Number of blades
pitchAngle = 2*np.pi/Nb
Nt = 10
theta = np.linspace(-0.5*pitchAngle, 0.5*pitchAngle,Nt)
...
nose = hubData[0:13] #This is the definition of the spinner nose. You can adjust as you wish, this is where I decide to cut mine
...
#The Tr point, is the r value as I move up along the nose surface, basically if you think of the butterfly as a kite or diamond, this point defines the top of the diamond, you can move it 
#up or down depending on what works for you. 

#So for the Tz, again, I chose this value arbitrarily, it is the point along the z that determines where the apex of the diamond lies, this is why I then interpolate the value to get the 
#value on the nose.
...
# hFunc = CubicSpline(hubData[:,1], hubData[:,0])
...
offCas = np.column_stack((cas[:,0], cas[:,1]-delThick))

offHub = np.column_stack((hub[:,0], hub[:,1]+delThick))

offsetHub = np.zeros((Nt,res+4,3))
offsetCas = np.zeros((Nt,res+4,3))
...
XhO = np.zeros([Nt, res+4])
YhO = np.zeros([Nt, res+4])
ZhO = np.zeros([Nt, res+4])
XcO = np.zeros([Nt, res+4])
YcO = np.zeros([Nt, res+4])
ZcO = np.zeros([Nt, res+4])
for d in range(Nt):
    for e in range(res+4):
...
rotInlet = np.zeros((Nt, rSections, 3))
rotOutlet = np.zeros((Nt, rSections, 3))
rotLE = np.zeros((Nt, newNsection, 3))
rotTE = np.zeros((Nt, newNsection, 3))
staLE = np.zeros((Nt, newNsection, 3))
staTE = np.zeros((Nt, newNsection, 3))
rotHub = np.zeros((Nt, len(hub), 3))
rotCas = np.zeros((Nt, len(cas), 3))
...
for b in range(Nt):
    ...
    rotCas[b,:,:] = np.array(vectorRotZ3D(cas[:,1],0,cas[:,0],theta[b])).T
    
#%%
Xrle = np.zeros([Nt, newNsection])
Yrle = np.zeros([Nt, newNsection])
Zrle = np.zeros([Nt, newNsection])
Xrte = np.zeros([Nt, newNsection])
Yrte = np.zeros([Nt, newNsection])
Zrte = np.zeros([Nt, newNsection])
Xsle = np.zeros([Nt, newNsection])
Ysle = np.zeros([Nt, newNsection])
Zsle = np.zeros([Nt, newNsection])
Xste = np.zeros([Nt, newNsection])
Yste = np.zeros([Nt, newNsection])
Zste = np.zeros([Nt, newNsection])

Xi = np.zeros([Nt, rSections])
Yi = np.zeros([Nt, rSections])
Zi = np.zeros([Nt, rSections])
Xo = np.zeros([Nt, rSections])
Yo = np.zeros([Nt, rSections])
Zo = np.zeros([Nt, rSections])
...
plt.plot(hub[:,0], hub[:,1], 'k')
# plt.plot(offHub[:,0], offHub[:,1], 'r')
# plt.plot()

#%%  Generate the stls 
# filenames = ['casing', 'Hub', 'LES', 'LE', 'TES', 'TE', 'mPer', 'nVal', 'pVal', 'outlet', 'inlet', 'offsetHub', 'offsetCas']
filenames = ['casing', 'Hub', 'LES', 'LE', 'TES', 'TE']

# Xvalues = [Xc, Xh, Xsle, Xrle, Xste, Xrte, Xm, Xn, Xp, Xo, Xi, XhO, XcO]
# Yvalues = [Yc, Yh, Ysle, Yrle, Yste, Yrte, Ym, Yn, Yp, Yo, Yi, YhO, YcO]
# Zvalues = [Zc, Zh, Zsle, Zrle, Zste, Zrte, Zm, Zn, Zp, Zo, Zi, ZhO, ZcO]

Xvalues = [Xc, Xh, Xsle, Xrle, Xste, Xrte]
Yvalues = [Yc, Yh, Ysle, Yrle, Yste, Yrte]
Zvalues = [Zc, Zh, Zsle, Zrle, Zste, Zrte]

for qq in range(len(Xvalues)):
    filename = '../thinSliceRotorOnly/constant/geometry/{}.stl'.format(filenames[qq])
...
blkLen2 = rLEData[0,0] - x0#nose[-1,0]
NbcX2 = 2*(Nbl)
blkFrac2 = k*delThick/blkLen2
r2, yn2 = commonRatio(y1, k*delThick, NbcX2)
# r2, yn2 = commonRatio(y1, blkLen2, NbcX2)
gradHBlk2 = yn2/y1
gradCBlk2 = y1/yn2
...
#for block7
# blkLen7 =(hub[-1,0] -domainExtn*2*casData[0,1]*percent) - sTEData[0,0] 
blkLen7 =hub[-1,0] - rTEData[0,0] 
NbcX7 = 3*(Nbl)
...
# blk7CellCount = 2*newNbcX7
# R7 = gradHBlk7**(1/(blk7CellCount-1))
# last7 = (blkLen7*(1-R7)*(R7**(blk7CellCount-1)))/(1-(R7**blk7CellCount))
#for block8
blkLen8 = hub[-1,0] - (hub[-1,0] -domainExtn*2*casData[0,1]*percent)
NbcX8 = 0.5*(Nbl)
blkFrac8 = 1#/blkLen8
r8, yn8 = commonRatio(y1, delThick, NbcX8)
# r8, yn8 = commonRatio(yn7, blkLen8, NbcX8)
# yn8 = 10*yn7
gradHBlk8 = yn7/yn8
gradCBlk8 = yn8/yn7
newNbcX8 = 1 + np.log(yn8/yn7)/np.log(r8)
HcoreBlk8 = blkLen8 #- yn7#delThick
NcoreBlk8 = HcoreBlk8/yn8
blk8CellCount = NcoreBlk8 + newNbcX8
# blk8CellCount = blkLen8/last7 
...
x1y0m = [inletR[0], 0, hub[0,0]-delThick]
x2y0m = [inletR[0], 0, hub[0,0]]

x0y1m = [T, 0, inletX[0]]
x0y1p = [noseMid*xAngP, noseMid*yAngP, inletX[0]]
x0y1n = [noseMid*xAngN, noseMid*yAngN, inletX[0]]
x1y1m = [T, 0, Zt-delThick]
x1y1p = [noseMid*xAngP, noseMid*yAngP, Zmid-delThick]
x1y1n = [noseMid*xAngN, noseMid*yAngN, Zmid-delThick]
x2y1m = [T, 0, Zt]
x2y1p = [noseMid*xAngP, noseMid*yAngP, Zmid]
x2y1n = [noseMid*xAngN, noseMid*yAngN, Zmid]
x0y2m = [nose[-1,1]+delThick, 0, inletX[0]]
x0y2p = [(nose[-1,1]+delThick)*xAngP, (nose[-1,1]+delThick)*yAngP, inletX[0]]
x0y2n = [(nose[-1,1]+delThick)*xAngN, (nose[-1,1]+delThick)*yAngN, inletX[0]]

# x2y2m = [Xm[-1][0], Ym[-1][0], nose[-1,0]]

x2y2m = [Xm[-1][0], Ym[-1][0], x0]
x2y2p = [Xm[-1][0]*xAngP, Xm[-1][0]*yAngP, x0]
x2y2n = [Xm[-1][0]*xAngN, Xm[-1][0]*yAngN, x0]

x0y4m = [cas[:,1][0]-delThick, 0, cas[:,0][0]]
x0y4p = [(cas[:,1][0]-delThick)*xAngP, (cas[:,1][0]-delThick)*yAngP, cas[:,0][0]]
x0y4n = [(cas[:,1][0]-delThick)*xAngN, (cas[:,1][0]-delThick)*yAngN, cas[:,0][0]]

# x2y3m = [Xm[-1][0]+delThick, Ym[-1][0], nose[-1,0]]
x2y3m = [Xm[-1][0]+delThick, Ym[-1][0], x0]
x2y3p = [(Xm[-1][0]+delThick)*xAngP, (Xm[-1][0]+delThick)*yAngP, x0]
x2y3n = [(Xm[-1][0]+delThick)*xAngN, (Xm[-1][0]+delThick)*yAngN, x0]

x0y5m = [cas[:,1][0], 0, cas[:,0][0]]
x0y5p = [cas[:,1][0]*xAngP, cas[:,1][0]*yAngP, cas[:,0][0]]
x0y5n = [cas[:,1][0]*xAngN, cas[:,1][0]*yAngN, cas[:,0][0]]

# x2y4m = [rotorInlet-delThick, 0, nose[-1,0]]
x2y4m = [rotorInlet-delThick, 0, x0]
x2y4p = [(rotorInlet-delThick)*xAngP, (rotorInlet-delThick)*yAngP,  x0]
x2y4n = [(rotorInlet-delThick)*xAngN, (rotorInlet-delThick)*yAngN,  x0]

# x2y5m = [rotorInlet, 0,nose[-1,0]] 
x2y5m = [rotorInlet, 0, x0]
x2y5p = [rotorInlet*xAngP, rotorInlet*yAngP, x0]
x2y5n = [rotorInlet*xAngN, rotorInlet*yAngN, x0]

xSy0m = [hub[0,1], 0, nose[0,0]-domainExtn*2*casData[0,1]*percent]

xSy1m = [T, 0, nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy1p = [noseMid*xAngP, noseMid*yAngP, nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy1n = [noseMid*xAngN, noseMid*yAngN, nose[0,0]-domainExtn*2*casData[0,1]*percent]

xSy2m = [nose[-1,1]+delThick, 0,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy2p = [(nose[-1,1]+delThick)*xAngP, (nose[-1,1]+delThick)*yAngP,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy2n = [(nose[-1,1]+delThick)*xAngN, (nose[-1,1]+delThick)*yAngN,  nose[0,0]-domainExtn*2*casData[0,1]*percent]

xSy4m = [inviscidR-delThick, 0,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy4p = [(inviscidR-delThick)*xAngP, (inviscidR-delThick)*yAngP,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy4n = [(inviscidR-delThick)*xAngN, (inviscidR-delThick)*yAngN,  nose[0,0]-domainExtn*2*casData[0,1]*percent]

xSy5m = [inviscidR, 0,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy5p = [inviscidR*xAngP, inviscidR*yAngP,  nose[0,0]-domainExtn*2*casData[0,1]*percent]
xSy5n = [inviscidR*xAngN, inviscidR*yAngN,  nose[0,0]-domainExtn*2*casData[0,1]*percent]


# x3y2m = [hub[-1,1], 0, hub[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y2m = [exitRh, 0, hub[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y2p = [exitRh*xAngP, exitRh*yAngP, hub[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y2n = [exitRh*xAngN, exitRh*yAngN, hub[-1,0]-domainExtn*2*casData[0,1]*percent]

# x3y3m = [hub[-1,1]+delThick, 0, hub[-1,0]-domainExtn*2*casData[0,1]*percent]
x3y3m = [hub[-1,1]+delThick, 0, hub[-1,0]]
x3y3p = [(hub[-1,1]+delThick)*xAngP, (hub[-1,1]+delThick)*yAngP, hub[-1,0]]
x3y3n = [(hub[-1,1]+delThick)*xAngN, (hub[-1,1]+delThick)*yAngN, hub[-1,0]]

# x4y2m = [hub[-1,1], 0, hub[-1,0]]
# x4y2m = [exitRh, 0, hub[-1,0]]
# x4y2p = [exitRh*xAngP, exitRh*yAngP, hub[-1,0]]
# x4y2n = [exitRh*xAngN, exitRh*yAngN, hub[-1,0]]

x3y2m = [hub[-1,1], 0, hub[-1,0]]
x3y2p = [hub[-1,1]*xAngP, hub[-1,1]*yAngP, hub[-1,0]]
x3y2n = [hub[-1,1]*xAngN, hub[-1,1]*yAngN, hub[-1,0]]

# # x4y3m = [hub[-1,1]+delThick, 0, hub[-1,0]]
# x4y3m = [exitRh+delThick, 0, hub[-1,0]]
# x4y3p = [(exitRh+delThick)*xAngP, (exitRh+delThick)*yAngP, hub[-1,0]]
# x4y3n = [(exitRh+delThick)*xAngN, (exitRh+delThick)*yAngN, hub[-1,0]]

# x3y4m = [cas[-1,1]-delThick, 0, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y4m = [exitRc-delThick, 0, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y4p = [(exitRc-delThick)*xAngP, (exitRc-delThick)*yAngP, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y4n = [(exitRc-delThick)*xAngN, (exitRc-delThick)*yAngN, cas[-1,0]-domainExtn*2*casData[0,1]*percent]

# # x3y5m = [cas[-1,1], 0, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y5m = [exitRc, 0, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y5p = [exitRc*xAngP, exitRc*yAngP, cas[-1,0]-domainExtn*2*casData[0,1]*percent]
# x3y5n = [exitRc*xAngN, exitRc*yAngN, cas[-1,0]-domainExtn*2*casData[0,1]*percent]

# x4y4m = [exitRc-delThick, 0, cas[-1,0]]
# x4y4p = [(exitRc-delThick)*xAngP, (exitRc-delThick)*yAngP, cas[-1,0]]
# x4y4n = [(exitRc-delThick)*xAngN, (exitRc-delThick)*yAngN, cas[-1,0]]
...
# x4y4m = [cas[-1,1]-delThick, 0, cas[-1,0]]
# x3y4m = [exitRc-delThick, 0, cas[-1,0]]
# x3y4p = [(exitRc-delThick)*xAngP, (exitRc-delThick)*yAngP, cas[-1,0]]
# x3y4n = [(exitRc-delThick)*xAngN, (exitRc-delThick)*yAngN, cas[-1,0]]

# x4y5m = [cas[-1,1], 0, cas[-1,0]]
# x4y5m = [exitRc, 0, cas[-1,0]]
...
x0y2ArcP =  [(nose[-1,1]+delThick)*np.cos(np.deg2rad(pAng*0.5)), (nose[-1,1]+delThick)*np.sin(np.deg2rad(pAng*0.5)), inletX[0]]
x0y2ArcN =  [(nose[-1,1]+delThick)*np.cos(np.deg2rad(nAng*0.5)), (nose[-1,1]+delThick)*np.sin(np.deg2rad(nAng*0.5)), inletX[0]]

#Define Grading properties 
# blRadial = Nr
blRadial = np.round(totalCellCount)
# coreRadial = np.round(Ncore)
radial = np.round(xdN*blRadial)
coreRadial = np.round(blRadial - 2*radial)
#%%

f = open('../thinSliceRotorOnly/system/pointData', 'w')
